...
<link href="${ctx }/h-ui/css/H-ui.min.css" rel="stylesheet" type="text/css" />
<link href="${ctx }/h-ui/css/H-ui.login.css" rel="stylesheet" type="text/css" />
<link href="${ctx }/h-ui/lib/icheck/icheck.css" rel="stylesheet" type="text/css" />
<link href="${ctx }/h-ui/lib/Hui-iconfont/1.0.1/iconfont.css" rel="stylesheet" type="text/css" />
		...
		$("#submitBtn").click(function(){
			...
			$.ajax({
				...
				success: function(msg){
					...
					if("loginError" == msg){
						$.messager.alert("消息提醒", "对不起,您输入的用户名或密码错误!", "warning");
						/* $("#vcodeImg").click();//切换验证码
						$("input[name='vcode']").val("");//清空验证码输入框 */
					}else if("typeError" == msg){
						$.messager.alert("消息提醒", "对不起,您登陆的身份错误!", "warning");
					}else if("admin" == msg){
						window.location.href = "${ctx}/sys/toAdminView.action";
					} else if("student" == msg){
						window.location.href = "${ctx}/sys/toStudentView.action";
					} else if("teacher" == msg){
						window.location.href = "${ctx}/sys/toTeacherView.action";
					}
				}
...
<div class="footer">Copyright &nbsp; SWU @ zsy </div>
