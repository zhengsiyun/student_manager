	...
	$(function() {	
        	...
        	if(selectLength == 0){
            	...
            	$.messager.confirm("消息提醒", "将删除与课程相关的所有数据，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "${ctx}/teacher/deleteTeacher.action",
					...
					handler:function(){
						...
						if(!validate){
							...
							$(table).find(".chooseTr").each(function(){
								...
								var course = gradeid+","+clazzid+","+courseid;
	  ...
	  //下拉框通用属性
	  	...
	  	$("#add_courseList").combobox({
	  		url: "${ctx}/course/queryAllCourseNotPage.action",
	  		onLoadSuccess: function(){
		  		//默认选择第一条数据
				var data = $(this).combobox("getData");;
				$(this).combobox("setValue", data[0].id);
				...
				},
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							$.ajax({
								type: "post",
								url: "${ctx}/teacher/updateTeacher.action",
								data: data,
								success: function(msg){
									if(msg == "success"){
...
<body>
	...
	<div id="addDialog" style="padding: 10px;">  
   		<div style=" position: absolute; margin-left: 560px; width: 250px; height: 300px; border: 1px solid #EEF4FF" id="photo">
    		<img alt="照片" style="max-width: 250px; max-height: 300px;" title="照片" src="${ctx }/photo/teacher.jpg" />
	    </div> 
	    	...
	    	<table id="addTable" border=0 style="width:800px; table-layout:fixed;" cellpadding="6" >
	    		<tr>
	    			...
	    			<td colspan="3">
	    				...
	    				<input type="hidden" name="photo" value="${ctx }/photo/teacher.jpg">
	    			</td>
