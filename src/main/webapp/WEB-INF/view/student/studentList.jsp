...
<head>
	...
	$(function() {	
	    ...
	    $('#dataList').datagrid({ 
	        ...
	        fit: true,//自动大小 
	        method: "post",
	        url:"${ctx}/student/queryAllStudent.action",
	        idField:'id', 
	    ...
	    $("#delete").click(function(){
        	...
        	if(selectLength == 0){
            	...
            	var number = selectRow.number;
            	var id = selectRow.id;
            	$.messager.confirm("消息提醒", "将删除与学生相关的所有数据(包括成绩)，确认继续？", function(r){
            		if(r){
            			$.ajax({
							type: "post",
							url: "${ctx}/student/deleteStudent.action",
	  	...
	  	$("#gradeList").combobox({
	  		...
	  		editable: false, //不可编辑
	  		method: "post",
	  		url: "${ctx}/grade/queryAllGradeNotPage.action",
	  	...
	  	$("#clazzList").combobox({
	  		...
	  		editable: false, //不可编辑
	  		method: "post",
	  		url: "${ctx}/clazz/queryAllClazzNotPage.action",
	  		onChange: function(newValue, oldValue){
	  			//加载班级下的学生
...
<body>
	...
	<div id="addDialog" style="padding: 10px">  
		<div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF" id="photo">
	    	<img alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="${ctx }/photo/student.jpg" />
	    </div> 
	    	...
	    	<table cellpadding="8" >
	    		<tr>
	    			...
	    			<td>
	    				...
	    				<input type="hidden" name="photo" value="${ctx }/photo/student.jpg">
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
	...
	<div id="editDialog" style="padding: 10px">
		<%-- <div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF">
	    	<img id="edit_photo" alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="${ctx }/photo/student.jpg" />
	    </div>    --%>
	    <div style="float: right; margin: 20px 20px 0 0; width: 200px; border: 1px solid #EBF3FF" id="photo">
	    	<img alt="照片" style="max-width: 200px; max-height: 400px;" title="照片" src="${ctx }/photo/student.jpg" />
	    </div> 
    	<form id="editForm" method="post">
	    	...
	    	<tr>
	    			<td>ID:</td>
	    			<td><input id="edit_id" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="id" readonly="readonly"/>
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>学号:</td>
	    			<td>
	    				...
	    				<input type="hidden" name="photo" value="photo/student.jpg">
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>姓名:</td>
	    			
	    			<td><input id="edit_name" style="width: 200px; height: 30px;" class="easyui-textbox" type="text" name="name" data-options="required:true, missingMessage:'请填写姓名'" />
	    			</td>
	    		</tr>
	    		<tr>
	    			<td>性别:</td>
